...
public class TenantLayerProperties {
     ...
     * row-level security rejects tenant-scoped work outside a transaction while leaving shared
     * registry reads available, and schema-per-tenant raises an unresolved-relation error. All
     * are safe, but switching is not behaviour-preserving on the no-tenant or no-transaction
     * paths.
     */
